...
Data collection is ubiquitous in many areas of research. Recently, more and more data collection has been automated to facilitate subsequent computer analysis. The same applies to access to subjects' interpretations, performed in the form of self-reports. Usually collected in paper form as questionnaires, these self-reports are standard in several applications in the field of psychology and have been used in studies of pain, emotion, behavioral, anxiety, and personality assessment, for example.
...
<!---Self-reports -> VAS-->
Because of its practicality and substantial body of research support, self-reports are one of the most popular tools to measure personality [@robins2009handbook]. Especially, VASs are used as simplistic tools to provide single-index measures, with a majority of research using VASs focusing on obtaining health states [@aastrom2023use]. Examples include measuring pain [@heller2016analyze] but also various other applications such as estimation of hunger [@beaulieu2021psychobiology] or feelings, such as fear (of childbirth) [@rouhe2009fear] or anxiety [@van2008validation].
...
Although several works have proven that scores obtained from computer– and mobile phone–based platforms and traditional paper-based VAS assessment are not interchangeable [@byrom2022measurement] but retrieve comparable results [@byrom2022measurement; @whybrow2006evaluation; @kos2017electronic; @delgado2018validation], electronic implementations of VASs are not commonly applied.
<!--- eVAS are not freely accessible-->
In more details, paper using electronic versions rarely share their software, such as @turnbull2020comparison, and open-source code to implement solutions is rare.
<!--- Available solutions-->
Moreover, research articles introducing electronic scales are outdated with broken download links, like the Adaptive Visual Analog Scales (AVAS) [@marsh2009adaptive], or for specific use cases, such as the visual analog scales measuring state anxiety (VAS-A) [@abend2014reliability].

<!--- Introduction our solution-->
Thus, we introduce **eVAS**, an open source Python software to easily realise an electronic VAS.
It is available with built applications for all operating systems (Windows, Linux, MacOS), highly configurable and easy-to-use.
Detailed description of the introduced scale can be found in the following sections.
...
<!--- CSV outcome --->
Measured feedback from the eVAS is stored in a CSV file for ease of use. The resulting files consist of two columns called 'secs' and 'values', giving an entry with the measurement and an associated timestamp in seconds.
The delimiter and floating point are set to the European standard, a semicolon and comma respectively, but can be changed in the configuration. In addition, the frequency and whether values should be saved only on change can also be adjusted in the config.ini file. The timestamp in seconds is stored to two decimal places, while the values are freely configurable.
...
<!--- Log file --->
Although the eVAS was implemented to catch errors, unexpected errors are logged into a "log.txt" file. If no error is logged, the log file is removed if it is empty at the end of the eVAS instance. Common errors, such as an faulty config.ini file, are caught and useful information given by message boxes to the end user. A screenshot of an example of warning displayed when the configuration file is setup wrongly, can be seen in \autoref{fig:config_warning}.

![Warning if the config file is setup wrong.\label{fig:config_warning}](../images/config_warning.png){ width=50% }

<!--- Update -->
To enable automatic deployment of new features and bug fixes, the software checks at the startup whether new releases are available on GitHub. If a new version is present, the user is informed and an automated update can be performed (Windows only).

<!--- Github | PyInstaller -->
Researchers are welcome to contribute on GitHub, open pull request, implement their own versions and deploy them using the supplied scripts to build the executables via PyInstaller. The application and scripts have been implemented and tested using Python 3.12.

<!--- Creation & Target Group -->
The idea and first implementations of eVAS were realised in the course of the BMBF project _PainMonit_ ('Multimodale Plattform zum Schmerz-Monitoring in der Physiotherapie', 01DS19008A/B). Here, the task of building automated pain detection systems in physiotherapy contexts showed a clear need for a digital yet easy-to-use tool. As we focused on providing a simple application that could be used by researchers of any background, we did not make the software available on the Python Package Index or related repositories. Knowledge of Python should not be a requirement for end users, so eVAS is available as standalone software running on Windows, MacOS and Ubuntu. Previous implementations of the software have been used to collect data for pain-related studies [@luebke2024differential; @szikszay2023psychological; @szikszay2022temporal].
<!--- Conclusion -->
By creating an easy to use Python software, we hope that eVAS can help researcher to easily realise their experiments and collect subjective feedback from subjects.
